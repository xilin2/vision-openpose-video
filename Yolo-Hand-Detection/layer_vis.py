...
class VGGhook():
    ...
    def __call__(self, oriImg, layer_to_hook):
        ...
        try:
            ...
            out = hook.output
            if self.device.type != 'cuda':
                out = out.clone().detach().requires_grad_(True).detach().numpy()
...
#                        ADD FLATTEN BACK
#                        return_data = return_data.flatten()
                        ...
                        for _ in range(square):
                            ...
                            for _ in range(square):
                                ...
                                ax.set_xticks([])
                                ax.set_yticks([])
                                plt.imshow(return_data[0, ix-1, :, :], cmap='gray')
                                ix += 1
                        
                        plt.suptitle('Visualization of Layer {} in {} - 64/{} total filters'.format(layer+1, self.model, return_data.shape[1]))
                        plt.savefig('visualizations/layer{}_{}'.format(layer+1, self.model))
                        plt.cla()
